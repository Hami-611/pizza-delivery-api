...
class OrderCreateListView(generics.GenericAPIView):
    ...
    @swagger_auto_schema(operation_summary="List all of orders made")
    def get(self,request):
...
class OrderDetailView(generics.GenericAPIView):
    ...
    @swagger_auto_schema(operation_summary="Delete the order")    
...
class UserOrderDetail(generics.GenericAPIView):
    ...
    @swagger_auto_schema(operation_summary="Get a user's specific order details")
    def get(self, request, user_id, order_id):  
        user = get_object_or_404(User, pk=user_id)
        order = get_object_or_404(Order, customer=user, pk=order_id)  
        serializer = self.serializer_class(instance=order)
        return Response(data=serializer.data, status=status.HTTP_200_OK)
